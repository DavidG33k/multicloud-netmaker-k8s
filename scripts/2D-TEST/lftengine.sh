...
output_file="${MACHINE_INFO}_lft_results.txt"

i=0
while [ "$i" -lt $num_of_tests ]; do
    timestamp=$(date +"%Y-%m-%d %H:%M:%S")
    
    result=$(lft -H 50 $HOSTNAME_OR_IP) # REMEMBER! lft needs root permission, so run it with sudo!
    ...
    echo "Lft results updated! $timestamp"
